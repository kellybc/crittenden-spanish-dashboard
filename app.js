...
async function loadShared(){try{const r=await fetch('/api/progress',{cache:'no-store'});if(!r.ok)throw new Error();const data=await r.json();members=data.members;history=data.history||[];render()}catch{render()}}
...
$('#entryForm').addEventListener('submit',async e=>{
  ...
  try{const r=await fetch('/api/progress',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(update)});const data=await r.json();if(!r.ok)throw new Error(data.error||'Could not save');members=data.members;history=data.history||[];render();$('#pinInput').value='';$('#entryDialog').close()}catch(err){$('#formMessage').textContent=err.message}finally{button.disabled=false;button.firstChild.textContent='Save for everyone '}
});
render();loadShared();
