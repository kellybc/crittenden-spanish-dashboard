...
async function read(sql){
  ...
  const courseWeekly=await sql`WITH bounds AS (SELECT (DATE_TRUNC('week',(NOW() AT TIME ZONE 'America/Chicago')-INTERVAL '4 days 8 hours')+INTERVAL '4 days 8 hours') AT TIME ZONE 'America/Chicago' starts_at), latest AS (SELECT DISTINCT ON (username,course_id) username,course_id,title,learning_language,xp FROM course_snapshots ORDER BY username,course_id,captured_at DESC), scores AS (SELECT l.name,l.username,l.emoji,l.accent,l.soft,c.course_id,c.title,c.learning_language,c.xp current_xp,b.starts_at,COALESCE((SELECT s.xp FROM course_snapshots s WHERE s.username=c.username AND s.course_id=c.course_id AND s.captured_at<=b.starts_at ORDER BY s.captured_at DESC LIMIT 1),c.xp) baseline FROM latest c JOIN learners l ON l.username=c.username CROSS JOIN bounds b) SELECT name,username,emoji,accent,soft,course_id,title,learning_language,current_xp,GREATEST(0,current_xp-baseline)::INTEGER weekly_xp,starts_at,starts_at+INTERVAL '7 days' ends_at FROM scores ORDER BY name,(learning_language='es') DESC,weekly_xp DESC,title`;
